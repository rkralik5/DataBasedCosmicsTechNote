...
The reconstructed data is influenced by the detector structure, reconstruction effieciencies and other effects that can bias the simulation. To avoid this influence, we apply smearing. Smearing is done by randomly changing
\begin{itemize}
...
We need to tell the \texttt{Text File Generator} whether to simulate a muon or an antimuon. However we do not reconstruct the charge of the muons, so we have to randomly assign it based on a statistical distribution from external measurments \cite{NOVA-doc-51327-v3}:
...
The start of track comparison between data and simulation on figures \ref{figDataMCComparison_startXstartY} and \ref{figDataMCComparison_startZ} show that there is fewer events that start at the edge of thedetector. This is likely the result of the event smearing.
